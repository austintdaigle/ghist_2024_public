...
def compute_summary_stats(vcf_path, window_length, output_path, max_dist_ld=1000, bin_size_ld=100, sequence_length=2.5e8):
    ...
    try:
        ...
        # Define indices for our new individuals in the VCF
        pop1_indices = [i for i, name in enumerate(sample_names) if name.startswith('pop1_')]
        pop2_indices = [i for i, name in enumerate(sample_names) if name.startswith('pop2_')]
        neanderthal_indices = [i for i, name in enumerate(sample_names) if name.startswith('AncSite')]
        
        # Further divide neanderthal_indices into ancient sites
        ancient_sites = {}
        for name in set([name.split('_')[0] for name in sample_names if name.startswith('AncSite')]):
            site_indices = [i for i, sname in enumerate(sample_names) if sname.startswith(name)]
            ancient_sites[name] = site_indices
        
        # Genotypes for each population
        genotypes_pop1 = genotypes[:, pop1_indices]
        genotypes_pop2 = genotypes[:, pop2_indices]
        genotypes_neanderthal = genotypes[:, neanderthal_indices]

    except Exception as e:
        print(f"Error reading VCF file: {e}")
        sys.exit(1)

    # Compute summary statistics for pop1, pop2, and neanderthal populations
    stats_pop1 = compute_population_stats(
        genotypes=genotypes_pop1,
        positions=positions,
        label='pop1',
    ...
    stats_pop2 = compute_population_stats(
        genotypes=genotypes_pop2,
        positions=positions,
        label='pop2',
        max_dist_ld=max_dist_ld,
        bin_size_ld=bin_size_ld,
    ...
    stats_neanderthal = compute_population_stats(
        genotypes=genotypes_neanderthal,
        positions=positions,
        label='neanderthal',
        max_dist_ld=max_dist_ld,
        bin_size_ld=bin_size_ld,
    ...
    # Compute pairwise statistics between populations
    stats_pairwise_pop1_pop2 = compute_pairwise_stats(
        genotypes1=genotypes_pop1,
        genotypes2=genotypes_pop2,
        positions=positions,
        label='pop1_pop2'
    )

    stats_pairwise_pop1_neanderthal = compute_pairwise_stats(
        genotypes1=genotypes_pop1,
        genotypes2=genotypes_neanderthal,
        positions=positions,
        label='pop1_neanderthal'
    )

    stats_pairwise_pop2_neanderthal = compute_pairwise_stats(
        genotypes1=genotypes_pop2,
        genotypes2=genotypes_neanderthal,
        positions=positions,
        label='pop2_neanderthal'
    )

    # Compute pairwise statistics among Neanderthal ancient sites
    stats_ancient_site_pairwise = {}
    ancient_site_names = list(ancient_sites.keys())
    for i in range(len(ancient_site_names)):
        for j in range(i+1, len(ancient_site_names)):
            site1 = ancient_site_names[i]
            site2 = ancient_site_names[j]
            genotypes_site1 = genotypes[:, ancient_sites[site1]]
            genotypes_site2 = genotypes[:, ancient_sites[site2]]
            label = f"{site1}_{site2}"
            stats = compute_pairwise_stats(
                genotypes1=genotypes_site1,
                genotypes2=genotypes_site2,
                positions=positions,
                label=label
            )
            stats_ancient_site_pairwise.update(stats)

    # Compute summary stats comparing each ancient site to pop1 and pop2
    stats_ancient_vs_pop = {}
    for site_name, site_indices in ancient_sites.items():
        genotypes_site = genotypes[:, site_indices]
        # Compare ancient site to pop1
        label = f"{site_name}_pop1"
        stats = compute_pairwise_stats(
            genotypes1=genotypes_site,
            genotypes2=genotypes_pop1,
            positions=positions,
            label=label
        )
        stats_ancient_vs_pop.update(stats)
        # Compare ancient site to pop2
        label = f"{site_name}_pop2"
        stats = compute_pairwise_stats(
            genotypes1=genotypes_site,
            genotypes2=genotypes_pop2,
            positions=positions,
    ...
    all_stats = {}
    all_stats.update(stats_pop1)
    all_stats.update(stats_pop2)
    all_stats.update(stats_neanderthal)
    all_stats.update(stats_pairwise_pop1_pop2)
    all_stats.update(stats_pairwise_pop1_neanderthal)
    all_stats.update(stats_pairwise_pop2_neanderthal)
    all_stats.update(stats_ancient_site_pairwise)
    all_stats.update(stats_ancient_vs_pop)
...
def compute_population_stats(genotypes, positions, label, max_dist_ld, bin_size_ld, window_length, sequence_length):
    ...
    n = genotypes.n_samples * 2  # Total number of chromosomes
...
def compute_pairwise_stats(genotypes1, genotypes2, positions, label):
    ...
    # Ensure that both genotype arrays have the same variants
    assert genotypes1.shape[0] == genotypes2.shape[0], "Genotype arrays must have the same number of variants."
    ...
    mean_fst = np.sum(num) / np.sum(den) if np.sum(den) > 0 else np.nan

    # Compute Weir and Cockerham's Fst
    genotypes_combined = allel.GenotypeArray(np.concatenate([genotypes1, genotypes2], axis=1))
    subpops = [
    ...
    mean_fst_wc = np.sum(a) / (np.sum(a) + np.sum(b) + np.sum(c)) if (np.sum(a) + np.sum(b) + np.sum(c)) > 0 else np.nan

    # Compute Patterson's F2 statistic
    f2 = allel.patterson_f2(ac1, ac2)
    mean_f2 = np.nanmean(f2)

    # Use mean_pairwise_difference_between to compute mean pairwise differences (pi) between populations
    pairwise_diffs = allel.mean_pairwise_difference_between(ac1, ac2)
    mean_pi = np.nanmean(pairwise_diffs)

    stats = {
        f'{label}_fst_hudson_mean': mean_fst,
        f'{label}_fst_weir_cockerham_mean': mean_fst_wc,
        f'{label}_f2_mean': mean_f2,
        f'{label}_pi_mean': mean_pi
    }
...
def main():
    ...
    parser.add_argument("--sequence_length", type=float, default=2.5e8, help="Total sequence length")
