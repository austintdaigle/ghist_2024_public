...
def compute_summary_stats(vcf_path, window_length, output_path, max_dist_ld=1000, bin_size_ld=100, sequence_length=1e8):
    ...
    try:
        ...
        # Check and subset populations if they exceed 18 individuals
        #if len(east_indices) > 18:
        #    print("East population exceeds 18 individuals; selecting a random subset of 18.")
        #    east_indices = np.random.choice(east_indices, 18, replace=False)
        #if len(west_indices) > 18:
        #    print("West population exceeds 18 individuals; selecting a random subset of 18.")
        #    west_indices = np.random.choice(west_indices, 18, replace=False)
    ...
    # Compute summary statistics for joint, east, and west populations
    ...
    stats_east = compute_population_stats(
        genotypes=genotypes_east,
        positions=positions,
        label='east',
        max_dist_ld=max_dist_ld,
        bin_size_ld=bin_size_ld,
    ...
    stats_west = compute_population_stats(
        genotypes=genotypes_west,
        positions=positions,
        label='west',
        max_dist_ld=max_dist_ld,
        bin_size_ld=bin_size_ld,
    ...
    # Compute pairwise statistics between east and west populations
    stats_pairwise = compute_pairwise_stats(
        genotypes1=genotypes_east,
        genotypes2=genotypes_west,
        positions=positions,
        label='east_west'
    )
    ...
    all_stats.update(stats_east)
    all_stats.update(stats_west)
